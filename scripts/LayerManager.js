...
define([
    ...
    // Influenza Menu
    LayerManager.prototype.influList = function () {
        const influName = [
            'InfluA',
            'InfluB']

        let influDropdown = $("#InfluA");

        let influOptions = $('<button class="btn btn-info btn-block dropdown-toggle" type="button" data-toggle="dropdown">Influenza A<span class="caret"></span></button>');

        influDropdown.append(influOptions);

        let ulItem3 = $('<ul class="dropdown-menu">');
        influDropdown.append(ulItem3);

        for (let i = 0; i < influName.length; i++) {
            let influItem = $('<li><a >' + influName[i] + '</a></li>');
            ulItem3.append(influItem);
        }
